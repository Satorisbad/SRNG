...
fn collect_use_targets(doc:&XmlDocument<'_>)->HashMap<String,UseTarget>{
    let nodes=doc.descendants().filter(|n|n.is_element()).filter_map(|n|n.attribute("id").map(|id|(id.to_string(),n))).collect::<HashMap<_,_>>();
    let mut memo=HashMap::new();
    for id in nodes.keys(){let mut visiting=HashSet::new();if let Some(target)=resolve_use_target(id,&nodes,&mut memo,&mut visiting){memo.insert(id.clone(),target);}}
    memo
}
fn resolve_use_target<'a>(id:&str,nodes:&HashMap<String,Node<'a,'a>>,memo:&mut HashMap<String,UseTarget>,visiting:&mut HashSet<String>)->Option<UseTarget>{
    if let Some(v)=memo.get(id){return Some(v.clone());}
    if !visiting.insert(id.to_string()){return None;}
    let node=*nodes.get(id)?;
    let target=if node.tag_name().name()=="use"{
        let href=node.attribute("href").or_else(||node.attribute(("http://www.w3.org/1999/xlink","href")))?;
        let base=resolve_use_target(href.strip_prefix('#')?,nodes,memo,visiting)?;
        UseTarget{data:base.data,fill:node.attribute("fill").and_then(solid_color).unwrap_or(base.fill),stroke:node.attribute("stroke").and_then(solid_color).unwrap_or(base.stroke)}
    }else{
        UseTarget{data:shape_path(node)?,fill:node.attribute("fill").and_then(solid_color).unwrap_or_else(||"#000000".into()),stroke:node.attribute("stroke").and_then(solid_color).unwrap_or_else(||"none".into())}
    };
    visiting.remove(id);memo.insert(id.to_string(),target.clone());Some(target)
}
...
#[test]fn resolves_nested_use_shape(){let svg=r##"<svg xmlns="http://www.w3.org/2000/svg"><defs><rect id="base" width="3" height="4" fill="#f00"/><use id="alias" href="#base"/></defs><use href="#alias" x="5" y="6"/></svg>"##;let r=import_svg(svg,"use.svg",&ImportOptions::default());assert!(r.source.contains("use-data:"));assert!(r.source.contains("use-fill: #ff0000;"));}
}
