...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompositeOperator { Over, In, Out, Atop, Xor }
...
pub fn parse_filter_graph(raw: &str, region: FilterRegion) -> Result<(FilterGraph, Vec<RenderDiagnostic>), String> {
    ...
    for statement in raw.split(';').map(str::trim).filter(|s| !s.is_empty()) {
        ...
        let op = match name {
            ...
            "composite" | "feComposite" => FilterPrimitive::Composite {
                input,
                input2: fields.get("in2").map(|v| parse_input(v)).unwrap_or(FilterInput::SourceGraphic),
                operator: parse_composite(fields.get("operator").or_else(|| fields.get("op")).map(String::as_str).unwrap_or("over"))?,
            },
            ...
            "merge" | "feMerge" => {
                let values = fields.get("inputs").map(|v| v.split(',').map(|s| parse_input(s.trim())).collect()).unwrap_or_else(|| positional_tokens(args).into_iter().map(|s| parse_input(&s)).collect());
            ...
            other => {
                diagnostics.push(RenderDiagnostic { severity: "warning".into(), code: "G241".into(), message: format!("unsupported filter primitive `{other}` is isolated and skipped"), declaration: None });
                FilterPrimitive::Unsupported { name: other.to_string() }
            }
        };
...
mod tests {
    ...
    #[test] fn unsupported_is_diagnostic_not_error(){let (g,d)=parse_filter_graph("feTurbulence()->noise",FilterRegion::full(20,20)).unwrap();assert_eq!(d.len(),1);assert!(matches!(g.nodes[0].op,FilterPrimitive::Unsupported{..}));}
}
