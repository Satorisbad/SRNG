...
impl fmt::Display for GpuRenderError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Scene(diagnostics) => write!(f, "scene preparation produced {} renderer diagnostic(s)", diagnostics.len()),
...
impl GpuRenderer {
    pub fn new(device:&Device,target_format:TextureFormat,target_width:u32,target_height:u32)->Self{
        let config=RenderTargetConfig{format:target_format,width:target_width.max(1),height:target_height.max(1)};
    ...
    pub fn render_to_view(&mut self,scene:&PreparedScene,device:&Device,queue:&Queue,encoder:&mut CommandEncoder,view:&TextureView)->Result<(),GpuRenderError>{
        let width=u32::from(scene.width);let height=u32::from(scene.height);
        if width>self.target_width||height>self.target_height{return Err(GpuRenderError::Backend(format!("prepared scene {width}x{height} exceeds renderer target {}x{}",self.target_width,self.target_height)));}
        let hybrid_scene=build_scene(scene).map_err(GpuRenderError::Scene)?;let render_size=RenderSize{width,height};let texture_bindings=TextureBindings::new();
        self.renderer.render(&hybrid_scene,&mut self.resources,device,queue,encoder,&render_size,view,&texture_bindings).map_err(|error|GpuRenderError::Backend(error.to_string()))
    }
}
...
pub fn build_scene(scene:&PreparedScene)->Result<HybridScene,Vec<RenderDiagnostic>>{
    ...
    for command in &scene.commands{if let Err(message)=apply(&mut output,command){diagnostics.push(RenderDiagnostic{severity:"error".into(),code:"G600".into(),message,declaration:None});}}
    if diagnostics.iter().any(|d|d.severity=="error"){Err(diagnostics)}else{Ok(output)}
}

fn apply(context:&mut HybridScene,command:&Command)->Result<(),String>{match command{
    ...
    Command::PushMaskSvg{..}|Command::PopMask=>return Err("SVG rasterized mask layers are currently implemented by the CPU renderer; GPU mask texture support is still pending".into()),
...
fn stops(values:&[crate::GradientStop])->ColorStops{ColorStops(values.iter().map(|stop|ColorStop{offset:stop.offset,color:color(stop.color).into()}).collect())}
