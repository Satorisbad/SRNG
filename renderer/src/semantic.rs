...
///
/// Native vector pattern/mask resources are reconstructed in memory; the SRNG
/// source itself does not need SVG-prefixed metadata or embedded SVG/XML.
pub fn prepare_scene(scene: &Scene, revision: u64, gate: &RevisionGate) -> PreparedScene {
    ...
    let has_preserved_defs = normalized.nodes.iter().any(|node| {
        node.active
            && node
                .properties
                .get("svg-source-tag")
    ...
    if !has_preserved_defs {
        if let Some(defs) = native_defs_xml(&normalized) {
...
fn bridge_properties(properties: &mut BTreeMap<String, String>) {
    ...
    // Pattern metrics are inspected from the preserved pattern XML by the
    // existing renderer. Generate that compatibility payload from native data
    // when no provenance payload exists.
