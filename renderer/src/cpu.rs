...
fn render_commands(context:&mut RenderContext,commands:&[Command],width:u16,height:u16)->Result<(),String>{
    ...
    while index<commands.len(){
        match &commands[index]{
            ...
            Command::PushFilter{graph}=>{let end=matching_end(commands,index,|c|matches!(c,Command::PushFilter{..}),|c|matches!(c,Command::PopFilter),"filter")?;let layer=render_layer(&commands[index+1..end],width,height)?;let filtered=crate::filter_cpu::execute_filter_graph(&layer,graph,width,height)?;composite_pixmap(context,filtered,width,height)?;index=end+1;}
...
fn percent_decode(input:&str)->Result<Vec<u8>,String>{let bytes=input.as_bytes();let mut out=Vec::with_capacity(input.len());let mut i=0;while i<bytes.len(){if bytes[i]==b'%'{if i+2>=bytes.len(){return Err("truncated percent escape in data URI".into());}let h=hex(bytes[i+1])?;let l=hex(bytes[i+2])?;out.push((h<<4)|l);i+=3;}else{out.push(bytes[i]);i+=1;}}Ok(out)}
