...
pub fn prepare_scene(scene: &Scene, revision: u64, gate: &RevisionGate) -> PreparedScene {
    ...
    for node in &scene.nodes {
        if node.active { ordered.push((node.paint_order, Source::Node(node))); }
    }
    for reference in &scene.references {
        if reference.active { ordered.push((reference.paint_order, Source::Reference(reference))); }
    }
    ...
    for (_, source) in ordered {
        ...
        let Some(path) = path_for(kind, &geometry, &props) else {
            let (code, message) = if kind == "text" {
                ("G301", format!("text node `{id}` requires pre-shaped outline path data in `data`"))
            } else {
                ("G101", format!("`{id}` has no renderable geometry for kind `{kind}`"))
            };
        ...
        if let Some(records) = native_mask.as_ref() {
            ...
            if legacy_mask.is_some() {
                diagnostics.push(diag("warning", "G224", "mask is using legacy SVG compatibility data; reimport to native v0.5 resources".to_string(), id));
            }
        }
        ...
        if legacy_mask.is_some() {
            diagnostics.push(diag("error", "G225", "legacy SVG masks are no longer executable in the native v0.5 command model".to_string(), id));
        }
...
fn path_for(kind: &str, geometry: &Geometry, props: &BTreeMap<String, String>) -> Option<PathData> {
    if let Some(data) = props.get("data") {
        ...
        if !value.trim().is_empty() { return Some(PathData { svg: value }); }
    }
    ...
    match kind {
        ...
        "ellipse" | "circle" => {
            let rx = width / 2.0; let ry = height / 2.0; let cx = x + rx; let cy = y + ry;
            Some(PathData { svg: format!("M {} {cy} A {rx} {ry} 0 1 0 {} {cy} A {rx} {ry} 0 1 0 {} {cy} Z", cx-rx, cx+rx, cx-rx) })
        }
        _ => None,
    }
}

fn native_records(props: &BTreeMap<String, String>, key: &str) -> Result<Option<Vec<VectorRecord>>, String> {
    let Some(data) = props.get(key).map(|value| unquote(value)) else { return Ok(None); };
    let mut records = Vec::new();
    for (index, line) in data.lines().enumerate() {
        if line.trim().is_empty() { continue; }
        let Some((paint, path)) = line.split_once('|') else {
            return Err(format!("{key} record {} is missing `paint|path` separator", index + 1));
        };
        let paint = parse_color(paint.trim()).map(Paint::Solid)
            .map_err(|error| format!("{key} record {}: {error}", index + 1))?;
        if path.trim().is_empty() { return Err(format!("{key} record {} has an empty path", index + 1)); }
        records.push(VectorRecord { path: PathData { svg: path.trim().to_string() }, paint });
    }
    if records.is_empty() { return Err(format!("{key} contains no renderable records")); }
    Ok(Some(records))
}

fn legacy_mask_svg(props:&BTreeMap<String,String>,defs_xml:&str,geometry:&Geometry,viewport_width:u16,viewport_height:u16)->Result<Option<String>,String>{
    if props.get("mask-mode").or_else(||props.get("svg-mask-mode")).map(|v|unquote(v)).as_deref()==Some("binary-opaque-clip"){return Ok(None);}
    let raw=props.get("mask-ref").or_else(||props.get("svg-mask-ref")).or_else(||props.get("svg-attr-mask")).map(|v|unquote(v));
    let Some(raw)=raw else{return Ok(None);};
    let mask_ref=if let Some(value)=raw.strip_prefix("url(#").and_then(|value|value.strip_suffix(')')){value.to_string()}else if !raw.contains('(')&&!raw.trim().is_empty(){raw.clone()}else{return Ok(None);};
    if defs_xml.trim().is_empty(){return Err(format!("SVG mask `{mask_ref}` has no native mask-data and no preserved <defs> XML"));}
    let x=geometry.x.unwrap_or(0.0);let y=geometry.y.unwrap_or(0.0);let width=geometry.width.unwrap_or(f64::from(viewport_width));let height=geometry.height.unwrap_or(f64::from(viewport_height));let escaped_ref=xml_escape_attr(&mask_ref);
    Ok(Some(format!("<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"{viewport_width}\" height=\"{viewport_height}\"><defs>{defs_xml}</defs><rect x=\"{x}\" y=\"{y}\" width=\"{width}\" height=\"{height}\" fill=\"white\" mask=\"url(#{escaped_ref})\"/></svg>")))
}

fn pattern_paint(props:&BTreeMap<String,String>,defs_xml:&str,geometry:&Geometry)->Result<Option<Paint>,String>{
    let pattern_ref=props.get("pattern-ref").or_else(||props.get("svg-pattern-ref")).map(|v|unquote(v));let Some(pattern_ref)=pattern_ref else{return Ok(None);};
    let stored_width=props.get("pattern-width").or_else(||props.get("svg-pattern-width")).and_then(|v|parse_number(v)).filter(|v|v.is_finite()&&*v>0.0).ok_or_else(||format!("pattern `{pattern_ref}` has no positive tile width"))?;
    let stored_height=props.get("pattern-height").or_else(||props.get("svg-pattern-height")).and_then(|v|parse_number(v)).filter(|v|v.is_finite()&&*v>0.0).ok_or_else(||format!("pattern `{pattern_ref}` has no positive tile height"))?;
    let fallback_pattern=props.get("pattern-source").or_else(||props.get("svg-pattern-source-xml")).map(|v|unquote(v)).unwrap_or_default();
    let units=props.get("pattern-units").map(|v|unquote(v)).unwrap_or_else(||if fallback_pattern.contains("patternUnits=\"objectBoundingBox\""){"objectBoundingBox".into()}else{"userSpaceOnUse".into()});
    let(tile_width,tile_height)=if units=="objectBoundingBox"{(stored_width*geometry.width.unwrap_or(1.0).abs(),stored_height*geometry.height.unwrap_or(1.0).abs())}else{(stored_width,stored_height)};
    if !tile_width.is_finite()||!tile_height.is_finite()||tile_width<=0.0||tile_height<=0.0{return Err(format!("pattern `{pattern_ref}` resolves to an empty tile"));}
    if let Some(records)=native_records(props,"pattern-data")? { return Ok(Some(Paint::Pattern { records, tile_width, tile_height })); }
    if fallback_pattern.trim().is_empty()&&defs_xml.trim().is_empty(){return Err(format!("pattern `{pattern_ref}` has no native pattern-data"));}
    let definitions=if defs_xml.trim().is_empty(){format!("<defs>{fallback_pattern}</defs>")}else{defs_xml.to_string()};let escaped_ref=xml_escape_attr(&pattern_ref);
    let svg=format!("<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"{tile_width}\" height=\"{tile_height}\"><defs>{definitions}</defs><rect width=\"{tile_width}\" height=\"{tile_height}\" fill=\"url(#{escaped_ref})\"/></svg>");
    Ok(Some(Paint::SvgPattern{svg,tile_width,tile_height}))
}
...
fn parse_paint(value:&str,props:&BTreeMap<String,String>,geometry:&Geometry)->Result<Paint,String>{
    ...
    if value=="linear-gradient"||value.starts_with("linear-gradient("){
        let stops_text=props.get("gradient-stops").ok_or_else(||"linear gradient requires `gradient-stops`".to_string())?;let stops=parse_gradient_stops(stops_text)?;if stops.len()<2{return Err("linear gradient requires at least two stops".into());}
        let x=geometry.x.unwrap_or(0.0);let y=geometry.y.unwrap_or(0.0);let width=geometry.width.unwrap_or(1.0);let height=geometry.height.unwrap_or(0.0);
        let start=props.get("gradient-start").and_then(|v|parse_pair(v)).unwrap_or((x,y));let end=props.get("gradient-end").and_then(|v|parse_pair(v)).unwrap_or((x+width,y+height));return Ok(Paint::LinearGradient{start,end,stops});
    }
    if value=="radial-gradient"||value.starts_with("radial-gradient("){
        let stops_text=props.get("gradient-stops").ok_or_else(||"radial gradient requires `gradient-stops`".to_string())?;let stops=parse_gradient_stops(stops_text)?;if stops.len()<2{return Err("radial gradient requires at least two stops".into());}
        let x=geometry.x.unwrap_or(0.0);let y=geometry.y.unwrap_or(0.0);let width=geometry.width.unwrap_or(1.0).abs();let height=geometry.height.unwrap_or(1.0).abs();
        let cx=props.get("gradient-cx").and_then(|v|parse_number(v)).unwrap_or(x+width/2.0);let cy=props.get("gradient-cy").and_then(|v|parse_number(v)).unwrap_or(y+height/2.0);let fx=props.get("gradient-fx").and_then(|v|parse_number(v)).unwrap_or(cx);let fy=props.get("gradient-fy").and_then(|v|parse_number(v)).unwrap_or(cy);let radius=props.get("gradient-r").and_then(|v|parse_number(v)).unwrap_or(width.max(height)/2.0);if !radius.is_finite()||radius<=0.0{return Err("radial gradient radius must be a positive finite number".into());}return Ok(Paint::RadialGradient{center:(cx,cy),focal:(fx,fy),radius,stops});
    }
